...
resource "null_resource" "wait_for_guardium_ready" {
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      GUARDIUM_INSTANCE_PUBLIC_IP  = aws_instance.central_manager[count.index].public_ip
      GUARDIUM_INSTANCE_PRIVATE_IP = aws_instance.central_manager[count.index].private_ip
...
resource "null_resource" "configure_guardium" {
  ...
  provisioner "local-exec" {
    command = <<EOT
echo "============================================================"
echo "[INFO] Configuring Guardium Central Manager: ${each.value.hostname}"
...
echo "[INFO] Central Manager configuration complete for ${each.value.hostname}"
echo "============================================================"
EOT
  }
}
